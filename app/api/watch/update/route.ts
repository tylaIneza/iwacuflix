...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isFinite(watchTimeSecs) || watchTimeSecs < 0)
    return NextResponse.json({ message: 'Invalid watchTimeSeconds' }, { status: 400 });

  try {
    await WatchHistory.upsert({
      userId:            payload.userId,
      videoId,
      watchTimeSeconds:  Math.floor(watchTimeSecs),
